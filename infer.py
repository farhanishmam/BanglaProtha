...
def run_gemma3(questions: list[str], modality: str) -> ModelRequestData:
    assert modality == "image"
    model_name = "google/gemma-3-12b-it" #"google/gemma-3-12b-it"
...
def run_llava(questions: list[str], modality: str) -> ModelRequestData:
    assert modality == "image"
    model_name_or_path = "llava-hf/llava-1.5-7b-hf" # "llava-hf/llava-1.5-7b-hf"
...
def get_multi_modal_input(data, test_image_folder):
    ...
    for item in data:
        if item.get('prediction') is None:
            ...
            # Generate lettered options: A. চারুকলা, B. গান, ...
            option_letters = list(string.ascii_uppercase)
            formatted_options = [
                f"{letter}. {option}" for letter, option in zip(option_letters, options_list)
            ]
...
def main(args):
    ...
    # Save updated JSON
    output_json_path = f'{model}_test_set_predictions.json'
